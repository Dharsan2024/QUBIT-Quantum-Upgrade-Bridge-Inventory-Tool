...
| `qubit-scanner` (discovery) | **built** | **Five real sources:** code AST (tree-sitter, segfault-pinned `<0.26` with regression test; 152 rules across Python/Go/Java/JS/TS/C, covering key use as well as key generation, incl. PQC APIs in all of Go/Java/Python/JS/TS/C), config (**nginx, Apache, OpenSSH**), active TLS enumeration **+ a real raw-ClientHello PQC-group probe** (was a mock until 2026-08-15; now proven against a live OpenSSL 3.5 server), cert/key, **dependency/SCA manifests** (5 formats, version-aware capability gates), and an opt-in **HashiCorp Vault transit/PKI connector**. Evidence redaction; network-scan authorization guardrail + audit log | more language rule-packs and more curated SCA packages (breadth, ongoing) |
...
| `qubit-migrate` (orchestrator) | **built** | Dependency graph + SCC order; risk÷effort WSJF queue; Ollama LLM transformer + repair loop + preservation guard + prompt-injection/path hardening; deterministic template codemods; **5-stage docker-sandbox validation**; FSM + review inbox. **14 transform rules** spanning config (nginx/Apache/OpenSSH), dependency manifests and code across 6 languages, covering **100% of the vulnerable assets detection can produce** (measured by sweeping every detection rule's positive examples; was 31%) — matched on provenance, filename and suffix so a config codemod is never pointed at source. Rules mark a codemod `codemod_authoritative` where the correct output is a constant, so an LLM cannot displace the deterministic hybrid-PQC edit. **E1–E5 all landed:** migration KB (`migration_kb.yaml`, now incl. JWT `token`-context entries), agility policy, per-asset recommendation, graph API, governance gates | deterministic codemods for the cross-language kex/signature rewrites, which today depend on the local model (breadth, ongoing) |
