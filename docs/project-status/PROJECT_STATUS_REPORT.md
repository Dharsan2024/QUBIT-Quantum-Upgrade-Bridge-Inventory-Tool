...
| Test result | **688 passed · 0 failed · 0 skipped** | `uv run pytest packages -q` |
...
| `qubit-cli` (`qubit`) | **built** | `scan/risk/plan/migrate/cbom/project/db/serve/rules`; `demo run --all` chaining the software loop + bridge; `scan-network`; **`scan-vault`**; `serve token create\|list\|revoke`; `qubit risk eval`; UTF-8 console reconfigure | `--server` client mode (cut-line); PyPI publication (deferred) |
...
`uv run pytest packages -q` at `4984bb5`: **688 passed, 0 failed, 0 skipped.**
