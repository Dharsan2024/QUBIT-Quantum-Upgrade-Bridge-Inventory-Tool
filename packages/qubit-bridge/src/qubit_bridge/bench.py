...
def bench_group(
    ...
    for _ in range(n):
        ...
        cmd = [
            "docker", "run", "--rm", "--entrypoint", "", "nginx:alpine", "/bin/sh", "-c", shell_cmd
        ]
        
        import contextlib
        with contextlib.suppress(subprocess.TimeoutExpired):
            subprocess.run(
                cmd, input=b"", capture_output=True, timeout=10.0
            )
    ...
    return HandshakeMeasurement(
        ...
        handshake_ms_p95=(
            ...
            if len(samples_ms) >= 20 else (max(samples_ms) if samples_ms else 0.0)
        ),
