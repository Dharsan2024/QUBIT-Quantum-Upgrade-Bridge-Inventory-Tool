...
RUBY_SHAPES: dict[str, RubyShape] = {
    "signature": RubyShape(
        ...
        ops={
            ...
            "sig-roundtrip": 'record("sig-roundtrip", "positive", sk.verify(nil, sig, MSG) == true)',
            "sig-wrong-key": 'record("sig-wrong-key", "negative", sk2.verify(nil, sig, MSG) == false)',
            "sig-tampered-message": (
                'record("sig-tampered-message", "negative", sk.verify(nil, sig, MSG_TAMPERED) == false)'
            ),
            "sig-truncated": (
                'record("sig-truncated", "negative", sk.verify(nil, sig[0..-2], MSG) == false)'
            ),
            "sig-size-class": (
                'record("sig-size-class", "advisory", sig.bytesize > 1000, '
                '"#{sig.bytesize} bytes")'
            ),
        },
    ),
...
def parameter_set(patched_source: str, target_algorithm: str, family: str = "") -> str:
    ...
    **Filtered by family, because one file holds several.** `inkwell-esign`'s `internal.rb` contains
    ...
    if wanted is not None and not wanted.match(target_algorithm):
        ...
        # a name any provider accepts. Use the family's own default rather than a guaranteed failure.
...
def build(relation_set: object, patched_source: str, target_algorithm: str) -> RubyPlan:
    ...
    if shape is None:
        return RubyPlan(
            skip_reason=f"no Ruby shape has been probed for the {family!r} family"
        )

    wanted = [
        r.id
        for r in getattr(relation_set, "relations", ())
        if getattr(r, "id", None) in shape.ops
    ]
    if not wanted:
        return RubyPlan(
            skip_reason=f"none of the {family!r} relations has a probed Ruby form"
        )
...
#: passes, and its signature is a real 3309-byte ML-DSA signature so the size advisory reads correct.
    ...
    return (
        ...
        Control(
            ...
            rationale=(
                ...
                "bytes, so both the positive and the advisory pass; only distinct-inputs catches it."
            ),
        ),
        Control(
            ...
            expect="pass",
            rationale="A correct AEAD migration. Ruby rejects by RAISING, unlike its own signatures.",
        ),
        Control(
