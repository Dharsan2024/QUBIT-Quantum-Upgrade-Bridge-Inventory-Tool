...
def _rsa_pem(tmp_path):
    ...
    cert = (
        x509.CertificateBuilder().subject_name(sub).issuer_name(sub)
...
def test_tls_success():
    async def fake_open(*a, **kw): return (MagicMock(), _FakeWriter())
    async def run():
...
def test_tls_no_ssl_obj():
    class _NoSsl:
        def get_extra_info(self, key): return None
        def close(self): pass
        async def wait_closed(self): pass
    async def fake_open(*a, **kw): return (MagicMock(), _NoSsl())
    async def run():
        with patch('asyncio.open_connection', new=fake_open):
...
def test_tls_refused():
    async def fake_open(*a, **kw): raise ConnectionRefusedError
    async def run():
        with patch('asyncio.open_connection', new=fake_open):
...
def test_tls_timeout():
    async def fake_open(*a, **kw): raise TimeoutError
    async def run():
        with patch('asyncio.open_connection', new=fake_open):
